"""PFD v2 gallery -- aggressive fractional-anchor deformation, no TS.

When TotalSegmentator returns empty masks on our cropped pelvic cache (OOD
for its training distribution), we fall back to fractional anchors but with
tighter Gaussians + POP-Q III-IV magnitudes so the structural deformation
is unambiguous. Default output: raw deformed real anatomy (--no-refine).

Output: synthetic_dataset/_pfd_v2/<patient_id>/
Dashboard: http://127.0.0.1:8766/  (same UI as training, parallel port)
...
from src.pfd_deformation import build_pattern_v2, build_displacement_field, apply_deformation
...
def make_stages(roster):
    ...
    for idx, (pat, sev) in enumerate(roster, start=1):
        ...
        out.extend([
            ...
            (f"{prefix}_field",  f"{label} - build v2 field"),
...
def main():
    ...
    ap.add_argument("--refine", action="store_true",
                    help="Also run the CVAE refine step (slower, smooths deformation seams). "
                         "Default is RAW deform on sharp real anatomy.")
    ...
    out_root = Path(cfg["paths"]["outputs_dir"]) / "_pfd_v2"
    ...
    url = wp.start_server(
        ...
        run_label=f"PFD v2 (aggressive fractional)  {refine_tag}  {len(GALLERY)} patients",
        expected_total_s=(len(GALLERY) * (85.0 if args.refine else 8.0)), cfg=None,
    )
    print()
    print("=" * 72)
    print(f"  v2 dashboard:        {url}")
    ...
    if args.refine:
        ...
        wp.log_msg("v2 default: RAW deformation on sharp real anatomy (no CVAE)")
    ...
    for idx, (pattern_name, severity) in enumerate(GALLERY, start=1):
        prefix = f"p{idx}"
        patient_id = f"PFD_V2_{pattern_name.upper()}_{severity.upper()}"
        ...
        pattern = build_pattern_v2(pattern_name, (Z, H, W), severity=severity)
        ...
        meta.update({
            "synthetic_strategy":
                "hybrid_v2_refined" if args.refine else "hybrid_v2_raw",
            ...
            "anchor_mode": "fractional_v2 (aggressive)",
        })
    ...
    wp.log_msg(f"v2 GALLERY complete: {len(summary)} patients in {total_min:.1f} min")
    print()
    print("=" * 72)
    print(f"  V2 GALLERY DONE in {total_min:.1f} min")
    for s in summary:
        if s.get("skipped"):
