"""PFD Option C gallery -- TotalSegmentator on ORIGINAL DICOM, then crop+resample
masks to cache coordinates. Deformation is anchored to ACTUAL organ centroids.

This is the proper Phase-1 path. It avoids the OOD failure of running TS on the
cropped pelvic cache by:
  1. Reading the original full-volume DICOM via src/io_loaders (which is more
     tolerant than dicom2nifti, the reader TS uses internally).
  2. Saving the full volume as a NIfTI in real HU.
  3. Running TS on the full volume (~5-15 min/patient on CPU).
  4. Cropping + resampling the masks through the same preprocessing pipeline
     the cache went through (pelvic_z_range + pelvic_xy_bbox + pad + resize).
  5. Anchoring cystocele/rectocele/uterine deformation to the actual bladder /
     rectum / midpoint centroids.

Output: synthetic_dataset/_pfd_optc/<patient_id>/
Mask cache (persisted, instant on re-run): cache/masks/<dataset>/<uid>/

Default: RAW deformation on sharp anatomy (--refine to add CVAE smoothing).
CPU-only. Training keeps the GPU.
"""
...
def make_stages(roster):
    ...
    for idx, (pat, sev) in enumerate(roster, start=1):
        ...
        out.extend([
            ...
            (f"{prefix}_load",    f"{label} - load cache volume"),
            (f"{prefix}_segment", f"{label} - TS on original (cached after)"),
            (f"{prefix}_field",   f"{label} - mask-anchored field"),
            (f"{prefix}_deform",  f"{label} - apply warp"),
            (f"{prefix}_refine",  f"{label} - refine (optional)"),
...
def get_original_source_path(cache_npz: Path) -> Path:
    """Read the 'source' field from a cache .npz to find the original DICOM dir."""
    with np.load(cache_npz, allow_pickle=True) as npz:
        src = str(npz["source"])
    p = Path(src)
    if not p.is_absolute():
        # Repo-relative paths used by preprocessing
        p = Path(src)
    return p
...
def main():
    ...
    ap.add_argument("--refine", action="store_true",
                    help="Also run CVAE refine step (smooths deformation seams).")
    ap.add_argument("--skip-done", action="store_true")
    args = ap.parse_args()

    cfg = load_config(args.config)
    ckpt_path = Path(args.ckpt) if args.ckpt else Path(cfg["paths"]["checkpoints_dir"]) / "cvae_latest.pt"
    if args.refine and not ckpt_path.exists():
        raise SystemExit(f"CVAE checkpoint not found: {ckpt_path}")

    cache_root = Path(cfg["paths"]["cache_dir"])
    out_root = Path(cfg["paths"]["outputs_dir"]) / "_pfd_optc"
    out_root.mkdir(parents=True, exist_ok=True)

    wp.set_stages(make_stages(GALLERY))
    refine_tag = "+ CVAE refine" if args.refine else "RAW (no refine)"
    expected_s = len(GALLERY) * (15 * 60 if args.refine else 10 * 60)
    url = wp.start_server(
        port=args.port, open_browser=not args.no_browser,
        run_label=f"PFD Option C  TS-on-original  {refine_tag}  {len(GALLERY)} patients",
        expected_total_s=expected_s, cfg=None,
    )
    print()
    print("=" * 72)
    print(f"  Option C dashboard:  {url}")
    ...
    if args.refine:
        ...
        wp.log_msg("Option C default: RAW deformation on sharp real anatomy (no CVAE)")
    ...
    for idx, (pattern_name, severity) in enumerate(GALLERY, start=1):
        prefix = f"p{idx}"
        patient_id = f"PFD_OPTC_{pattern_name.upper()}_{severity.upper()}"
        ...
        with np.load(info["cache_path"], allow_pickle=True) as npz:
        ...
        wp.update_stage(postfix=f"Z={Z}  H={H}  W={W}  source={src_path}")
        wp.finish_stage("done")

        if src_path is None or not src_path.exists():
            wp.log_msg(f"[{idx}/{len(GALLERY)}] SKIP: no original DICOM at {src_path}")
            for suffix in ("segment", "field", "deform", "refine", "write"):
                wp.set_stage(f"{prefix}_{suffix}", postfix="skipped (missing original DICOM)")
                wp.finish_stage("error", error="no original DICOM")
            continue

        wp.set_stage(f"{prefix}_segment", postfix="loading original / TS")
        ...
        try:
            ...
            masks, stats = segment_original_volume(
                source_path=src_path, dataset=info["dataset"], uid=info["uid"],
            ...
            if "colon" in masks and masks["colon"].any():
                ...
                if rs is not None:
                    stats["rectum"] = rs
            voxel_summary = ", ".join(f"{k}={v.voxels}" for k, v in stats.items())
            wp.update_stage(postfix=f"done in {seg_t:.0f}s; {voxel_summary[:60]}")
            wp.log_msg(f"  voxels: {voxel_summary}")
            wp.finish_stage("done")
        except Exception as e:
            wp.update_stage(postfix=f"FAILED: {e}")
            wp.finish_stage("error", error=str(e))
            wp.log_msg(f"  [{idx}/{len(GALLERY)}] SEG FAILED: {e}")
            for suffix in ("field", "deform", "refine", "write"):
                wp.set_stage(f"{prefix}_{suffix}", postfix="skipped (seg failed)")
                wp.finish_stage("error", error="seg failed")
            continue
        ...
        if not args.refine:
            wp.set_stage(f"{prefix}_refine", postfix="skipped (RAW mode)")
            refined = deformed
            wp.finish_stage("done")
        else:
            ...
            wp.update_stage(current=Z, total=Z, postfix=f"refined in {(time.time()-t0)/60:.1f} min")
            wp.finish_stage("done")
        ...
        meta.update({
            "synthetic_strategy":
                "hybrid_optc_refined" if args.refine else "hybrid_optc_raw",
            "cvae_refinement": bool(args.refine),
            ...
            "cvae_checkpoint": (ckpt_path.name if args.refine else None),
        ...
        summary.append({"patient_id": patient_id, "n_slices": int(Z),
                        ...
                        "dir": str(pdir), "findings": pattern.findings,
                        "organ_voxels": {k: int(v.voxels) for k, v in stats.items()}})

    total_min = (time.time() - t_gallery) / 60.0
    wp.log_msg(f"OPTION C GALLERY complete: {len(summary)} patients in {total_min:.1f} min")
    print()
    print("=" * 72)
    print(f"  OPTION C GALLERY DONE in {total_min:.1f} min")
    for s in summary:
        if s.get("skipped"):
