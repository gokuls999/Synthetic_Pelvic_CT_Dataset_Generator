"""PFD Production v5 — Surgical edit of real CT data.

Architecture:
  * Take a REAL source CT from the downloaded datasets (bones, organs, fat all intact).
  * ONLY displace the pelvic floor organs (bladder / rectum / uterus-region).
  * Everything outside those organs — bones, muscles, fat, bowel — stays
    pixel-identical to the original.  No smearing of bone or surrounding tissue.
  * Smooth feathered boundary (Gaussian weight mask) prevents seam artefacts.
  * Population (plain/hilly) is assigned from actual pelvimetry:
      - Transverse diameter of pelvic inlet measured from TS hip masks
      - Plain (gynecoid)         : TD ≥ 108 mm
      - Hilly (android/anthropoid): TD <  108 mm
  * Clinical POP-Q grades 1-4 per South Indian literature.
  * Complete metadata PDF matching the Master Data Collection Sheet.

Run 2-3 samples first:
    python scripts/pfd_production_v5.py --num-patients 3 --port 8771

Then full 350:
    python scripts/pfd_production_v5.py --num-patients 350 --skip-done --port 8771
"""
...
def _make_comparison_strip(vol_orig: np.ndarray, vol_edited: np.ndarray,
    ...
    def _win(arr):
        return ((np.clip(arr.astype(np.float32), lo_u, hi_u) - lo_u)
...
def process_one(cfg: dict, cache_root: Path, out_root: Path,
    ...
    clinical["anchor_mode"]          = "confined_organ_plus_levator_v5"
...
def main():
    ...
    ap.add_argument("--out",          default="synthetic_dataset/production_v5_350")
    ap.add_argument("--num-patients", type=int, default=3,
                    help="Start with 3 for preview, then run 350")
    ap.add_argument("--n-plain",      type=int, default=None,
                    help="Override plain count (default: num_patients//2)")
    ap.add_argument("--n-hilly",      type=int, default=None,
                    help="Override hilly count (default: num_patients//2)")
    ...
    wp.set_stages([
        ("plan",    "Plan roster"),
        ("process", "Surgical edit: load real CT → confine → deform organs only → DICOM"),
        ("summary", "Summary"),
    ])
    url = wp.start_server(
        port=args.port, open_browser=not args.no_browser,
        run_label=(f"PFD v5 Surgical-edit  N={n_total}  "
                   f"plain={n_plain}/hilly={n_hilly}  sources={len(sources)}"),
        expected_total_s=None, cfg=None,
    )
    print()
    print("=" * 72)
    print(f"  PFD Production v5  —  Surgical organ-only edit")
    print(f"  Dashboard : {url}")
    print(f"  Output    : {out_root}")
    print(f"  Mode      : CONFINED deformation — bones/fat/muscle UNTOUCHED")
    print(f"  Sources   : {len(sources)} real pelvic CTs")
    print(f"  Patients  : {n_total}  ({n_plain} plain / {n_hilly} hilly)")
    print("=" * 72)
    print()
    ...
    progress_marker.write_text(json.dumps({
        "n_planned": len(roster), "version": "v5",
    ...
    print()
    print("=" * 72)
    print(f"  PFD v5 DONE  {total_min:.1f} min")
